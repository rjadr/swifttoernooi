...
st.set_page_config(page_title='Swift Hemelvaartsdagtoernooi', page_icon="", layout="wide")

with st.sidebar:
    choose = option_menu("kv Swift Hemelvaartsdagtoernooi", ["Toernooi informatie", "Wedstrijdschema", "Standen", "Plattegrond", "Wedstrijdreglement", "Contact"],
                         icons=['info-circle', 'calendar3', 'trophy', 'map', 'journal-check', 'envelope'],
...
if choose == "Wedstrijdschema":
   ...
   # st.markdown('### Wedstrijdschema')

    filters = {
        'teams_choice': st.multiselect(
            'Teams:', options=teams),
        'verenigingen_choice': st.multiselect(
            "Verenigingen:", options=verenigingen),
        'poules_choice': st.multiselect(
            'Poules:', options=poules),
        'rondes_choice': st.multiselect(
            'Rondes:', options=rondes),
        'scheidsrechters_choice': st.multiselect(
            'Scheidsrechters:', options=scheidsrechters)
    }

    def filter_df(filter_name, filter_value):
        if filter_name == 'teams_choice':
            return schema[['Thuis', 'Uit']].isin(filter_value).any(axis=1)
        elif filter_name == 'verenigingen_choice':
            return (schema['Thuis'] + ' ' + schema['Uit']).str.contains('|'.join(filter_value))
